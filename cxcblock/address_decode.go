...
import (
	"fmt"
	"github.com/assetsadapterstore/cxcblock-adapter/cxcblock_addrdec"
...
func (decoder *addressDecoder) PrivateKeyToWIF(priv []byte, isTestnet bool) (string, error) {
	...
	cfg := cxcblock_addrdec.CXC_mainnetPrivateWIFCompressed
	if decoder.wm.Config.IsTestNet {
		cfg = cxcblock_addrdec.CXC_testnetPrivateWIFCompressed
	}

	wif, _ := cxcblock_addrdec.Default.AddressEncode(priv, cfg)
...
func (decoder *addressDecoder) PublicKeyToAddress(pub []byte, isTestnet bool) (string, error) {
	cxcblock_addrdec.Default.IsTestNet = decoder.wm.Config.IsTestNet
	address, err := cxcblock_addrdec.Default.AddressEncode(pub)
	if err != nil {
		return "", err
	}
...
func (decoder *addressDecoder) RedeemScriptToAddress(pubs [][]byte, required uint64, isTestnet bool) (string, error) {
	...
	cfg := cxcblock_addrdec.CXC_mainnetAddressP2SH
	if decoder.wm.Config.IsTestNet {
		cfg = cxcblock_addrdec.CXC_testnetAddressP2SH
	}
	...
	address, _ := cxcblock_addrdec.Default.AddressEncode(pkHash, cfg)
...
func (decoder *addressDecoder) WIFToPrivateKey(wif string, isTestnet bool) ([]byte, error) {
	...
	priv, err := cxcblock_addrdec.Default.AddressDecode(wif, cfg)
	if err != nil {
		return nil, err
	}
